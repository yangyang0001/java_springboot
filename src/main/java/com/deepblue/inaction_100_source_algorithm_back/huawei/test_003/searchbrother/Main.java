package com.deepblue.inaction_100_source_algorithm_back.huawei.test_003.searchbrother;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNextLine()) {
            ...
            String brot = searchBrother(line);
     ...
     * @return
     */
    public static String searchBrother(String line) {

        List<String> brotherList = new ArrayList<>();

        String[] split = line.split(" ");
        String index  = split[split.length - 1];
        String targe  = split[split.length - 2];

        for(int i = 1; i < split.length - 1; i++) {
            boolean brother = isBrother(targe, split[i]);
            if(brother) {
                brotherList.add(split[i]);
            }
        }

        brotherList.sort((o1, o2) -> o1.compareTo(o2));

        return brotherList.size() + (brotherList.size() < Integer.valueOf(index) ? "" : "\n" + brotherList.get(Integer.valueOf(index) - 1));
    }

    public static boolean isBrother(String targe, String param) {

        if(targe.length() != param.length() || targe.equals(param)) {
            return false;
        }

        char[] s = targe.toCharArray();
        char[] j = param.toCharArray();
